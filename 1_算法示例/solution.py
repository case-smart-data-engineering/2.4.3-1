...
def get_page_rank(IOS, alpha, max_itrs, min_delta):
    ...
    for i in range(max_itrs):
        if np.shape(PR)[0] == 0:
            PR = np.full(shape=(N, 1), fill_value=1.0 / N)

        old_PR = PR
        PR = np.dot(A, PR) #迭代计算网页的PageRank值
        D = np.array([old - new for old, new in zip(old_PR, PR)]) # 计算新的PageRank值和上一次PageRank值的差值
        ret = [abs(e) < min_delta for e in D] #判断D中满足设定阈值的个数

        if ret.count(True) == N: #判断是否满足迭代条件
            times = i + 1
            break
